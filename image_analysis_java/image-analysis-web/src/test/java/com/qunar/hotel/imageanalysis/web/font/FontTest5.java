...
public class FontTest5 {
    ...
    public FontTest5 createCheckCode() {
        FontTest5 checkCode = new FontTest5();
    ...
    public static void main(String[] args) {
        FontTest5 t = new FontTest5();
        t = t.createCheckCode();
        String str = t.checkCodeStr;
        System.out.println(str);

        String filePatn = "./test5.jpg";
        t.createImgFile(filePatn);

//        t.setCheckCodeStr(t.createRandomCode());
//        t.setBuffImg(t.disturb());
    ...
    private String createRandomCode() {
        ...
        int codeY = height - 4;
        char[] codeSequence = {'图', '像', '数', '据', '国', '程'};

        Graphics2D graphics = graphicsInit();
        graphics.setColor(createColor());

        for (int i = 0; i < codeCount; i++) {
            strRand = String.valueOf(codeSequence[random.nextInt(6)]);
    ...
    private BufferedImage twistImage() {
        System.out.println("扭曲图片");
        double dMultValue = random.nextInt(7) + 1;// 波形的幅度倍数，越大扭曲的程序越高，一般为3
        double dPhase = random.nextInt(1);// 波形的起始相位，取值区间（0-2＊PI）
