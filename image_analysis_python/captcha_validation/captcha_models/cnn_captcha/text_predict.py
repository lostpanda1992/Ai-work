...
class TestBatch(CNN):
    ...
    def gen_captcha_text_image(self):
        ...
        captcha_image = captcha_image.resize((55, 55))
...
with open("./cnn_captcha/conf/sample_config.json", "r") as f:
...
with open("./cnn_captcha/tools/labels.json", "r") as f:
...
total = 100
tb = TestBatch(test_image_dir, char_set, './cnn_captcha/model/', total)
...
saver = tf.train.Saver()
sess =  tf.Session()
saver.restore(sess, tb.model_save_dir)
...
def predict(captcha_image):
    captcha_array = cv2.resize(captcha_image, (55, 55))
    test_image = tb.convert2gray(captcha_array)
    test_image = test_image.flatten() / 255
    predict = tf.argmax(tf.reshape(y_predict, [-1, tb.max_captcha, tb.char_set_len]), 2)
    text_list = sess.run(predict, feed_dict={tb.X: [test_image], tb.keep_prob: 1.})
    predict_text = text_list[0].tolist()
    p_text = ""
    for p in predict_text:
        p_text += str(tb.char_set[p])
